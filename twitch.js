...
export function connectToTwitch(channel, onMessage, onConnect, updateStatus) {
  ...
  socket.onopen = () => {
    // onConnectCallback should be called when the connection is actually open and ready.
...
function createBadgeIcons(badgesStr) {
  ...
  // Using a map for badge SVGs makes it easy to add or change them.
  const badgeSVGs = {
  ...
  // Using map/join is a more functional approach than building a string with forEach.
  return badgesStr.split(',')
    .map(part => {
      const badgeName = part.split('/')[0];
      return badgeSVGs[badgeName] || '';
    })
    .join('');
}
