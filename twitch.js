...
export function connectToTwitch(channel, onMessage, onConnect, updateStatus) {
  if (!channel) {
    updateStatus('error', 'Будь ласка, введіть назву каналу.');
    return;
  }
  ...
  onConnectCallback();
  updateStatusCallback('connecting', `Під'єднюємось до #${channel}...`);
  ...
  socket.onerror = (error) => {
    ...
    updateStatusCallback('error', 'Помилка підключення. Перевірте логи консолі');
  };
...
export function handleMessage(message) {
  ...
  if (parsedMessage && parsedMessage.command === 'PRIVMSG') {
    ...
    updateStatusCallback('error', "Не вдалось підключитись. Можливо каналу не існує.");
    socket.close();
  }
  return null;
}
